"""Script for running wandb sweep for object detection and classification over hyperparameters
- nms_threshold
- conf_threshold

To run the script, run the following command:
python wandb_sweep/rejection_network.py -cn config_path +ckpt=checkpoint_path +n=100
...
def main(config: DictConfig):
    ...
    partial_evaluate = partial(
        evaluate, model=model, dataloader=dataloader, device=device, expt_config=config
    )
