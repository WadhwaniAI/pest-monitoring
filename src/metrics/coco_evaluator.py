""" version ported from
    ...
    Notes:
        ...
        2) COCO's ground truths contain an 'area' attribute that is associated with
        the segmented area if segmentation-level information exists. While coco uses
        this 'area' attribute to distinguish between 'small', 'medium', and 'large'
        objects, this implementation simply uses the associated bounding box area
        to filter the ground truths.
        3) COCO uses floating point bounding boxes, thus, the calculation of the
        box area for IoU purposes is the simple open-ended delta (x2 - x1) * (y2 - y1).
...
def get_coco_summary(groundtruth_bbs, detected_bbs):
    ...
    groundtruth_bbs : list
        A list containing objects of type BoundingBox representing the
        ground-truth bounding boxes.
    detected_bbs : list
        A list containing objects of type BoundingBox representing
    ...
    def _evaluate(iou_threshold, max_dets, area_range):
        """evaluate a single IoU threshold

        Parameters
        ----------
        iou_threshold : float
            IoU threshold to use for evaluation
        max_dets : int
            maximum number of detections to use for evaluation
        area_range : list
            Area range to use for evaluation
        """
    ...
    full = {
        i: _evaluate(iou_threshold=i, max_dets=100, area_range=(0, np.inf)) for i in iou_thresholds
    }
    ...
    small = {
        i: _evaluate(iou_threshold=i, max_dets=100, area_range=(0, 32**2)) for i in iou_thresholds
    }
    ...
    max_det1 = {
        i: _evaluate(iou_threshold=i, max_dets=1, area_range=(0, np.inf)) for i in iou_thresholds
    }
    AR1 = np.mean(
        [
    ...
    max_det10 = {
        i: _evaluate(iou_threshold=i, max_dets=10, area_range=(0, np.inf)) for i in iou_thresholds
    }
    AR10 = np.mean(
        [
            x["TP"] / x["total positives"]
...
def get_coco_metrics(
    ...
    """Calculate the Average Precision and Recall metrics as in COCO's official implementation
        ...
        max_dets : int
            Upper bound on the number of detections to be considered
            for each class in an image.
    Returns:
    ...
    mAP = sum([v["AP"] for k, v in res.items() if k in _evals]) / len(_evals)
    return {"per_class": res, "mAP": mAP}
...
def _jaccard(a, b):
    """Compute the jaccard overlap of two sets of bounding boxes.
...
def _evaluate_image(dt, gt, ious, iou_threshold, max_dets=None, area_range=None):
    ...
    gt : list
        ...
        the ground-truth bounding boxes.
    iou_threshold : float
        The IOU threshold to use for matching detections to ground truths.
    max_dets : int
        The maximum number of detections to use for evaluation.
    area_range : list
        The range of image areas to use for evaluation.
    """
...
def _compute_ap_recall(scores, matched, NP, recall_thresholds=None):
    """This curve tracing method has some quirks that do not appear when
    only unique confidence thresholds are used (i.e. Scikit-learn's implementation),
    however, in order to be consistent, the COCO's method is reproduced.

    Parameters
    ----------
    scores : list
        A list containing the confidence scores of the detected bounding boxes.
    matched : list
        A list containing the matched status of the detected bounding boxes.
    NP : int
        The number of ground-truth bounding boxes.
    recall_thresholds : list
        A list containing the recall thresholds.
    """
    if NP == 0:
        return {
            "precision": None,
    ...
    # n_recalls = len(recall_thresholds) # wasn't being used...
