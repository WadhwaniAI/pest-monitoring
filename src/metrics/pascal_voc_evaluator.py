...
def calculate_ap_every_point(rec, prec):
    """Calculate the average precision based on the every point interpolation method.
    The definition of the average precision (area under the precision-recall curve) can be
    found at: https://en.wikipedia.org/wiki/Average_precision

    Parameters
    ----------
    rec : list
        A list of recall values.
    prec : list
        A list of precision values.
    """
    mrec = []
    mrec.append(0)
...
def calculate_ap_11_point_interp(rec, prec, recall_vals=11):
    """Calculate the average precision based on the 11-point interpolated average precision.
    ...
    prec : list
        A list of precision values.
    recall_vals : int
        The number of recall values to use for interpolation.
    """
    mrec = []
    # mrec.append(0)
    [mrec.append(e) for e in rec]
...
def get_pascalvoc_metrics(
    ...
    iou_threshold : float, optional
        IOU threshold to match detected and ground truth boxes, by default 0.5
    method : int, optional
        Interpolation type, by default MethodAveragePrecision.EVERY_POINT_INTERPOLATION
    generate_table : bool, optional
        flag to aditionally generate Dataframe, by default False

    Returns
    -------
    ret : dict
        Dictionary containing information and metrics of each class. The key represents the class
        and the values are:
...
def plot_precision_recall_curve(
    ...
    """Plot the Precision x Recall curve for a given dataset and IoU threshold.

    Parameters
    ----------
    results : dict
        the results obtained calling `compute_ap_per_class()`
    mAP : float
        the mAP obtained calling `compute_ap_per_class()`
    showInterpolatedPrecision : bool
        if True, it shows the interpolated precision curve
    savePath : str
        the path to save the graphic, if provided
    showGraphic : bool
        if True, it shows the graphic (which must be saved first)
    """
    result = None
    plt.close()
...
def plot_precision_recall_curves(
    ...
    """Plotting multiple precision x recall curves for a given dataset.
    ...
    showAP : bool
        If true, the average precision value will be shown in the title.
    showInterpolatedPrecision : bool
        If true, it will show in the plot the interpolated precision values.
    savePath : str
